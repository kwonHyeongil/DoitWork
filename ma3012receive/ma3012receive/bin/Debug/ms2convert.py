...
class MSEED:
    def __init__(self, NetCode, StationCode, Location, Channel, Npts, SamplingHz, StartTime, XData, YData, ZData, EW_HOME, EW_PARAMS, MSEEDSavePath,TANKSavePath):
        self.NetCode = NetCode;
        self.StationCode = StationCode + Location;
        self.Location = '--';
    ...
    def MSWrite(self):
        ...
        try:
            ...
            strMSEED_PATH_X = PathJoin(self.MSEEDSavePath, self.NetCode + '_' + self.StationCode + '_' + self.Channel[0] + '.mseed')
            strTANKFILE_PATH_X = PathJoin(self.TANKSavePath,self.NetCode + '_' + self.StationCode + '_' + self.Channel[0] + '.tank')
            strMSEED_PATH_Y = PathJoin(self.MSEEDSavePath, self.NetCode + '_' + self.StationCode + '_' + self.Channel[1] + '.mseed')
            strTANKFILE_PATH_Y = PathJoin(self.TANKSavePath,self.NetCode + '_' + self.StationCode + '_' + self.Channel[1] + '.tank')
            strMSEED_PATH_Z = PathJoin(self.MSEEDSavePath, self.NetCode + '_' + self.StationCode + '_' + self.Channel[2] + '.mseed')
            strTANKFILE_PATH_Z = PathJoin(self.TANKSavePath,self.NetCode + '_' + self.StationCode + '_' + self.Channel[2] + '.tank')
